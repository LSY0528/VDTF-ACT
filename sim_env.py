...
class BimanualViperXTask(base.Task):
    ...
    @staticmethod
    def get_tactile(physics): # tactile
        tactile_data = np.round(physics.data.sensordata * 100) / 100
    ...
    def get_observation(self, physics):
        ...
        obs['tactile'] = self.get_tactile(physics) # tactile
        ...
        near_top = 0.1
        far_top = 0.8#0.6
        depth_top = physics.render(height=480, width=640, camera_id='top', depth=True)
        depth_front = physics.render(height=480, width=640, camera_id='depth_front', depth=True)
        depth_front_clipped = np.clip(depth_front, near_front, far_front)
        depth_top_clipped = np.clip(depth_top, near_top, far_top)
        depth_top_normalized = (depth_top_clipped - near_top) / (far_top - near_top)
        depth_front_normalized = (depth_front_clipped - near_front) / (far_front - near_front)
        depth_top_normalized = depth_top_normalized[..., np.newaxis]
        depth_front_normalized = depth_front_normalized[..., np.newaxis]
        obs['depth'] = dict()
        obs['depth']['top'] = depth_top_normalized
        obs['depth']['front'] = depth_front_normalized
